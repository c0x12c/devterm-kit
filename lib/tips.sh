...
show_tips() {
  ...
  # ── Config files ─────────────────────────────────────────────────────────
  echo ""
  echo -e "  ${YELLOW}${BOLD}Config files${NC} — Split for safe customization"
  echo -e "  ${DIM}─────────────────────────────────────────────${NC}"
  echo -e "  ${RED}Before:${NC}  One ~/.zshrc — devterm overwrites your custom config"
  echo -e "  ${GREEN}After:${NC}   3-file split — your config is safe in ~/.zshrc.local"
  echo ""
  echo -e "  ${BOLD}Files:${NC}"
  echo -e "    ${BLUE}~/.zshrc${NC}          Loader (sources the other two — don't edit)"
  echo -e "    ${BLUE}~/.zshrc.devterm${NC}  devterm-managed (regenerated by setup.sh)"
  echo -e "    ${BLUE}~/.zshrc.local${NC}    ${GREEN}YOUR config${NC} — never touched by devterm"
  echo -e "  ${DIM}  Tip: alias 'zshrc' opens ~/.zshrc.local in your editor${NC}"
  ...
  # ── Multiplexer ─────────────────────────────────────────────────────────
  echo ""
  echo -e "  ${YELLOW}${BOLD}Multiplexer${NC} — tmux / zellij cheat sheet"
  echo -e "  ${DIM}─────────────────────────────────────────────${NC}"
  echo -e "  ${BOLD}tmux (if installed):${NC}"
  echo -e "    ${BLUE}Ctrl+A c${NC}       New window"
  echo -e "    ${BLUE}Ctrl+A |${NC}       Split pane vertical"
  echo -e "    ${BLUE}Ctrl+A -${NC}       Split pane horizontal"
  echo -e "    ${BLUE}Ctrl+A h/j/k/l${NC} Navigate panes (vim-style)"
  echo -e "    ${BLUE}Ctrl+A d${NC}       Detach session"
  echo -e "    ${BLUE}tmux a${NC}         Re-attach last session"
  echo ""
  echo -e "  ${BOLD}zellij (if installed):${NC}"
  echo -e "    ${BLUE}zellij${NC}         Start new session"
  echo -e "    ${BLUE}Alt+n${NC}          New pane"
  echo -e "    ${BLUE}Alt+h/j/k/l${NC}   Navigate panes"
  echo -e "    ${BLUE}Alt+[ / Alt+]${NC}  Switch tabs"
  echo -e "    ${BLUE}Ctrl+o d${NC}       Detach session"
  echo -e "    ${BLUE}zellij a${NC}       Re-attach last session"
  echo -e "  ${DIM}  Install: ./setup.sh --multiplexer tmux|zellij|both${NC}"
