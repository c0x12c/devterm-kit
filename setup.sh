...
main() {
  ...
  if [[ "$DEVTERM_OS" == "macos" ]]; then
    echo -e "  ${MOCHA_BLUE}1.${NC} ${BOLD}Set font in iTerm2:${NC}"
    echo "     Preferences → Profiles → Text"
    echo "     → Font: 'MesloLGS NF'  Size: 14"
    echo ""
    echo -e "  ${MOCHA_BLUE}2.${NC} ${BOLD}Apply color scheme:${NC}"
    echo "     Preferences → Profiles → Colors"
    echo "     → Color Presets → catppuccin-${DEVTERM_THEME}"
    echo ""
    echo -e "  ${MOCHA_BLUE}3.${NC} ${BOLD}Fully quit and reopen iTerm2${NC}"
    echo "     (Cmd+Q, not just close window)"
    echo ""
    echo -e "  ${MOCHA_BLUE}4.${NC} ${BOLD}Reload your shell:${NC}"
    echo "     exec zsh"
  else
